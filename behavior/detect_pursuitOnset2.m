function [pursuit_onset,rxnTime,msFlag,csFlag] = detect_pursuitOnset2(eyePos,stimOnset,jumpDuration,targSpeed,targDir,jumpType,plt)
% OBJECTIVE:
...
[b, a] = butter(2, 30 / (1000 / 2), 'low');

% Apply the filter to the data
filtered_eyePos = filtfilt(b, a, eyePos);
eyeVel = diff(filtered_eyePos) * 1000;
...
eyeAcc = NaN(1, length(eyeVel));  % NaN for boundary points where no acceleration can be computed

% Compute acceleration by dividing the velocity difference over 20ms
for i = (window_size + 1):(length(eyeVel) - window_size)
    eyeAcc(i) = (eyeVel(i + window_size) - eyeVel(i - window_size)) / (20e-3);  % 20ms difference
end

% Calculate the standard deviation of the velocity during the fixation period
fixation_velocity = eyeVel(stimOnset - 50:stimOnset + 50);
...
for i = stimOnset+50:length(eyeVel)-10
    if eyeVel(i) > velocity_threshold && all(eyeVel(i:i+9) > threshold_velocity)
        pursuit_onset = i;
        break;  % Exit the loop as soon as the condition is satisfied
    end
end

if sum(abs(eyeAcc(stimOnset-50:stimOnset+50))>1000) || sum(abs(eyeVel(stimOnset-50:stimOnset+50))>30)
    msFlag = 1;
    rxnTime = NaN;
    csFlag = NaN;
else
    ...
    if sum(abs(eyeAcc(stimOnset+csPre:stimOnset+csPost))>1000) || sum(abs(eyeVel(stimOnset+csPre:stimOnset+csPost))>30)
        csFlag = 1;
    else
        csFlag = 0;
        if rxnTime > csPost
...
if plt==1
    ...
    plot(x,eyeVel(stimOnset-100:stimOnset+500),'k-','linewidth',2)
    ylabel('velocity')
    prettyFig;
