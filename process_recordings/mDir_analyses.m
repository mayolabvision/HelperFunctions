...
data = tbl(isnan(tbl.REWARD),:);
dirs = sort(unique(data.angle));

preint = 0;
postint = 500;

[Th,Rh] = cellfun(@(q,r) cart2pol(q(1,r-preint:r+postint),q(2,r-preint:r+postint)), data.eyePos,num2cell(data.FIX_ON), 'uni', 0);

f1a = figure;
f1a.Position = [100 100 1400 900];

for t=1:size(data,1)
    polarplot(Th{t},Rh{t},'k')
    hold on
end
