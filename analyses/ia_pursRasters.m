            ...
            if ~exist(fullfile(FIG_PATH2, sprintf('%s_clust%04d_chan%03d.png', probe_label, clust, chans(u))), 'file') | isempty(FIG_PATH)
                ...
                for ang = 1:length(angles)
                    ...
                    if IS_FHC
                        ...
                        spks = spks(:,scode+1);
                        if isequal(ALIGN,'targ')
                            sptimes = cellfun(@(w,v) w-v(1), spks, num2cell(these_trls.PURSUIT_TARG_ON), 'uni', 0);
                        elseif isequal(ALIGN,'purs')
                            sptimes = cellfun(@(w,v) w-v(1), spks, num2cell(these_trls.pursuitOnset), 'uni', 0);
                        end
                        ...
                        if isequal(ALIGN,'targ')
                            sptimes = cellfun(@(w,v) w-v(1), cellfun(@(q) q{(clust+1)}, these_trls.(prb_name), 'uni', 0), num2cell(these_trls.PURSUIT_TARG_ON), 'uni', 0);
                        elseif isequal(ALIGN,'purs')
                            sptimes = cellfun(@(w,v) w-v(1), cellfun(@(q) q{(clust+1)}, these_trls.(prb_name), 'uni', 0), num2cell(these_trls.pursuitOnset), 'uni', 0);
                        end
                ...
                for dd = 1:length(speeds)
                    ...
                    if numel(angles) > 1
                        ...
                        str_title{dd} = sprintf('%d deg -- Dir: %0.2f, Sel: %0.2f', speeds(dd), visdp, visds);
                    end
                ...
                legend_labels = arrayfun(@(d) sprintf('%d deg', speeds(d)), 1:length(speeds), 'UniformOutput', false);
