function [dat,tbl] = format_dataTable(nev,out,task_name,varargin)
    % blah
    %
    %%% Required inputs: %%%
    %   nev    -    nev file
    ...
    addParameter(p, 'CONVERT_TO_TABLE', defaultConvertToTable, @islogical);
    ...
    if CONVERT_TO_TABLE
        ...
        trialMarkers = cellfun(@(t) cellfun(@(q,r) find(q == round(r(r(:,2)==t,3)*1000)), times_ms, tbl1.trialcodes, 'uni', 0), eventCodes, 'uni', 0)';
        trialMarkers = horzcat(trialMarkers{:});
        trialMarkers(cellfun('isempty',trialMarkers)) = {NaN};

        for m=1:length(eventNames)
            if sum(cellfun(@(q) size(q,1)>1, trialMarkers(:,m), 'uni', 1))>0
    ...
    dat = rmfield(dat, {'spiketimes','net_labels','pupil','diode','eyes'});
end